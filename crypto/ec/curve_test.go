...
func BatchMarshalRoundTrip(t *testing.T) {
	...
	samePoints, err := BatchUnmarshalPoints(marshaledPointList)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarshalZero(t *testing.T) {
	...
	t.Logf("%v", marshalled)
	c := NewBasePoint()
	err := c.Unmarshal(marshalled)
	if err != nil {
		t.Fatal(err)
	}
	if !c.Equals(p) {
		t.Fatalf("points came back different %v %v", c, p)
	}
}

func TestSize(t *testing.T) {
	marshalled := NewBasePoint().Marshal()
	if len(marshalled) != CurveElementSize {
		t.Fatalf("Marshalled length %v but expected %v", len(marshalled), CurveElementSize)
	}
	if CurveElementSize != 33 {
		t.Fatalf("Curve element size is %d", CurveElementSize)
	}
	max_scalar := NewScalarElement(CURVE_MOD)
	packing := make([]byte, ScalarElementSize)
	max_scalar.PackTo(packing)
	s := ZeroScalar()
	s.InterpretFrom(packing)
	if s.Int.Cmp(CURVE_MOD) != 0 {
		t.Fatalf("Scalar unpacking did not equal packing")
	}
	if len(packing) != ScalarElementSize {
		t.Fatalf("Expected size %v but got size %v", ScalarElementSize, len(packing))
	}
	if ScalarElementSize != 32 {
		t.Fatalf("Scalar element size is %d", ScalarElementSize)
	}
}
